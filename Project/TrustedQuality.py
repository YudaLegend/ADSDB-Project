        ...
        if i != '':
            try:
                ...
                if from_currency in currencyToEUR:
                    ...
                    discount_price.append(int(discount)/100*currencyToEUR[from_currency])
                else:
                    initial_price.append(c.convert(int(initial), from_currency, 'EUR')/100)
                    discount_price.append(c.convert(int(discount), from_currency, 'EUR')/100)
            except Exception as e:
                print(e)
...
def steamGameDetailsQuality(con):
    ...
    initial_price, discount_price = currencyConvert(df)
